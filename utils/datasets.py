...
class load_images2():  # for inference
    def __init__(self, path, batch_size=1, img_size=416):
        if os.path.isdir(path):
            image_format = ['.jpg', '.jpeg', '.png', '.tif']
            self.files = sorted(glob.glob('%s/*.*' % path))
    ...
    def __next__(self):
        ...
        if self.count == self.nB:
            raise StopIteration
        img_name = self.files[self.count].split('/')[-1].split('\\')[-1].replace('txt', 'jpg')
        img_path = 'D:/00-Data/rtpose_datasets/COCO/images/train2014/'+img_name
        ...
        return [img_path], 'E:/08-Github-resources/600-yolov3-master/yolov3-master/labels/train-person/'+img_name.replace('jpg', 'txt'), img
...
class load_images_and_labels():  # for training
    def __init__(self, path, batch_size=1, img_size=608, multi_scale=False, augment=False):
        ...
        self.label_files = [path.replace('images', 'labels').replace('.png', '.txt').replace('.jpg', '.txt') for path in
                            self.img_files]
        # with open('E:/08-Github-resources/mtcnn-pytorch/img_path.txt') as f:
        #     self.temp = f.readlines()
        #     self.temp = [path.replace('\n', '') for path in self.temp]
        #     self.nF2 = len(self.temp)
        #     # print(self.temp[0])
        #     self.img_files.extend(self.temp)
        #     self.temp_label =[''.join(path.split('/')[-2:]).replace('.jpg', '.txt') for path in self.temp]
        #     # print(self.temp_label[0])
        #     self.label_files.extend(self.temp_label)

        self.nF = len(self.img_files)  # number of image files
        self.nB = math.ceil(self.nF / batch_size)  # number of batches math.ceil向下取整
        # self.nB = math.ceil(self.nF2 / batch_size)
        self.batch_size = batch_size
        self.height = img_size
    ...
    def __iter__(self):
        ...
        # self.shuffled_vector_2 = np.random.permutation((self.nF2)) if self.augment else np.arange(self.nF2)
    ...
    def __next__(self):
        ...
        for index, files_index in enumerate(range(ia, ib)):
            ...
            label_path = 'D:/01-ComputerVisionEntries/10-HumanDetection/PyTorch-YOLOv3-master/labels/train/'+label_path.split('/')[-1]
            # print(label_path)
            # face_path =
            # print(label_path)
            # print(label_path)
            ...
            if os.path.isfile(label_path):
                ...
                labels[:, 1] = ratio * w * (labels0[:, 1] - labels0[:, 3] / 2) + padw  # 将归一化的坐标值--恢复--转换成resize之后的坐标值
        ...
        img_all = np.stack(img_all)[:, :, :, ::-1].transpose(0, 3, 1, 2)  # Channel first and cv2 to pytorch
