...
def plot_one_box(x, img, color=None, label=None, line_thickness=2):  # Plots one bounding box on image img
...
def build_targets(pred_boxes, pred_conf, pred_cls, target, anchor_wh, nA, nC, nG, batch_report):#, center_features):
    """
    returns nT, nCorrect, tx, ty, tw, th, tconf, tcls
    """
    nB = len(target)  # number of images in batch
    nT = [len(x) for x in target]  # torch.argmin(target[:, :, 4], 1)  # numbers of object per image
    ...
    for b in range(nB):
        ...
        if nTb > 1:
            ...
            # c_f.append((center_features[b, :, gj, gi], t[:, 0]))
            if len(t.shape) == 1:
                t = t.view(1, 5)
        else:
            if iou_best < 0.10:
    ...
    return tx, ty, tw, th, tconf, tcls, TP, FP, FN, TC#, c_f
